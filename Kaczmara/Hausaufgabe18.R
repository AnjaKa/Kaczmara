...
# zunächst die lineare Regression für die einzelnen unabhängigen Variablen.
...
# Berechnen Sie die zwei Regressionsmodelle für y ~ x1 und y ~ x2
 ...
 pyreg.x1 <- lm(y~x1, data=pyreg)
 summary(pyreg.x1)

 pyreg.x2 <- lm(y~x2, data=pyreg)
 summary(pyreg.x2)
...
# Regression y ~ x1 + x2 berechnen:
 ...
 lm.pyreg <- lm(y~x1+x2, data=pyreg)
 summary(lm.pyreg)

# Wie gut passt das lineare Modell zu den Daten? Schauen Sie sich die R^2 und 
# F-Werte an sowie auch die t-Werte für die einzelnen Prediktoren an. Glauben Sie, 
