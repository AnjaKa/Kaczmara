...
for(s in c(3,4,5) ){
  ...
  output <- paste("Bei einer Standabweichung von",s, "haben",ausgezeichnet*100,"% ausgezeichnet.")
   print(output)
  }
```
...
Gute Noten kommen wesentlich seltener vor als schlechte Noten.

Warum?

Im Durchfallbereich gibt es mehr Noten als im guten Bereich. Im guten bis sehr guten Bereich gibt es nur die Noten 13, 14 und 15. Zum schlechten Bereich  zählen aber die Noten 0 bis 5.
...
2. Wie viele Studenten bekommen zumindest 10 NP?
    ...
    `r (pnorm(15, mean=mu,sd=3)-pnorm(10, mean=mu, sd=3)*n` Studenten bekommen zumindest 10 Notenpunkte.

3. Wie viele Studenten bekommen weniger als 10 NP?

    `r (pnorm(10, mean=mu, sd=3)-pnorm(0, mean=mu, sd=3)*n` Studenten bekommen weniger als 10 Notenpunkte.

4. Wie viele Studenten bekommen weniger als 8 NP?

    `(pnorm(8,mean=mu,sd=3) - pnorm(1,mean=mu,sd=3) ) * n` Studenten bekommen weniger als 8 Notenpunkte.
...
Der kritische Wert für einen einseitigen $z$-Test ist `r qnorm(0.94)`.

Die kritischen Werte für einen zweiseitigen $z$-Test sind $\pm$ `r qnorm(0.97)`.
